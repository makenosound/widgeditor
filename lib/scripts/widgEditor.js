...
/* ICELAB: Removed, now handled by Symphony */
// var widgStylesheet = "extensions/widgeditor/lib/css/widgContent.css";
...
function widgEditor(replacedTextareaID)
{
	...
	/*
  * ICELAB: + "WidgEditor" removed to get working with Symphony structure
  *
	this.theExtraInput.id = this.theTextarea.id + "WidgEditor";
	this.theExtraInput.name = this.theTextarea.name + "WidgEditor";
	*/
	this.theExtraInput.id = this.theTextarea.id;
	this.theExtraInput.name = this.theTextarea.name;
  ...
  /*
  * ICELAB: Removed to get working with Symphony structure
  *
...
widgEditor.prototype.writeDocument = function(documentContent)
{
  ...
  /* ICELAB: Remove the stylesheet insertion (now handled by Symphony) */
	var documentTemplate = '\
		<html>\
			<head>\
				<style type="text/css">body { background: #fff; font: .75em/ 1.4em normal "Lucida Grande", Helvetica, Arial, Verdana, sans-serif; } </style>\
			</head>\
			<body id="iframeBody">\
				INSERT:CONTENT:END\
			</body>\
